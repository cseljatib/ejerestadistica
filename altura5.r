## Script: "altura5.r"
## Sobre?: Calculo de estadisticos de validacion, para dos modelos de altura
##  versus diametro. Se emplea un set de diferentes de los otros ejercicios.
##  Entre otras cosas, se: 
##   + calcula valores predichos y errores.
##   + calcula estadisticos RMSD, DIFA y DA.
##   + emplea la dataframe "biomass2"
##
...
b0.hat2<-coef(mod2)[1];b0.hat2
b1.hat2<-coef(mod2)[2]


#valor ajustado, de cada modelo
df$aju1 <- b0.hat1 + b1.hat1 * (df$dap)   #modelo 1
df$aju2 <- b0.hat2 + b1.hat2 * (1/df$dap) #modelo 2

#valor del error de prediccion, en m. 
df$e.aju1 <- df$atot - df$aju1 #modelo 1
df$e.aju2 <- df$atot - df$aju2 #modelo 2
head(df)

...
##+ Aplicacion con modelo de crecimiento de Michaelis-Menten y /
##  comparacion                                               /
## ----------------------------------------------------------/ 
##                                                          /
...
#ajuste modelo de crecimiento de Michaelis-Menten
mm.nls <- nls(peso~
                (alpha*edad)/(beta+edad),
              data = df,    trace=T,
              start = list(alpha=220,beta=50))

#Note que para el valor inicial de alpha utilizado
# es el maximo de la variable respuesta. Beta es aproximadamente
# la mitad del tiempo en alcanzar alpha
summary(mm.nls)
...
y.creci.mm <-predict(mm.nls,
                         data.frame(edad=t.fake))

plot(peso~edad,data=df, bty="l", 
     xlab="Edad (num.meses)",ylab="Peso (Kg)")
lines(t.fake,y.creci.mm, 
      lty=1, lwd = 2, col="red")


#y que tal si lo comparamos con el modelo logistico?
